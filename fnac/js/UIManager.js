...
class UIManager {
    constructor(game) {
        this.game = game;
        this.lastOxygenValue = null;
        this.lastTimeText = '';
        this.lastNightText = '';
        this.lastSceneSrc = '';
        this.lastSceneOffset = null;
        this.lastVentsVisible = null;
        this.lastCameraVisible = null;
        this.initElements();
    }
    ...
    update() {
        const oxygenValue = Math.floor(this.game.state.oxygen);
        if (this.lastOxygenValue !== oxygenValue) {
            this.powerValue.textContent = oxygenValue;
            this.lastOxygenValue = oxygenValue;
        }

        const timeText = `${this.game.state.currentTime === 0 ? 12 : this.game.state.currentTime} AM`;
        if (this.lastTimeText !== timeText) {
        ...
        const nightText =
            this.game.state.customNight && this.game.state.currentNight === 7
                ? '7'
                : String(this.game.state.currentNight);

        if (this.lastNightText !== nightText) {
            this.nightValue.textContent = nightText;
            this.lastNightText = nightText;
        }
        ...
        if (!this.game.state.cameraOpen) {
            ...
            if (this.game.assets.images[sceneKey]) {
                const sceneSrc = this.game.assets.images[sceneKey].src;
                if (this.lastSceneSrc !== sceneSrc) {
                    this.currentSceneImg.src = sceneSrc;
                    this.lastSceneSrc = sceneSrc;
                }
    ...
    updateHotspotVisibility(viewPosition) {
        ...
        if (ventsBtn) {
            const ventsVisible = viewPosition < 0.15;
            if (this.lastVentsVisible !== ventsVisible) {
                ventsBtn.style.opacity = ventsVisible ? '1' : '0';
                ventsBtn.style.pointerEvents = ventsVisible ? 'auto' : 'none';
                this.lastVentsVisible = ventsVisible;
            }
        }
    ...
    updateViewPosition(viewPosition) {
        ...
        if (this.lastSceneOffset !== offset) {
