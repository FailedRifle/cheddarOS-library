...
class Game {
    constructor() {
        ...
        this.epsteinModeStorageKey = 'fnae_epstein_mode_unlocked';
        this.epsteinSecretClickCount = 0;
        this.epsteinModeUnlocked = false;
        localStorage.removeItem(this.epsteinModeStorageKey);
        this.viewPosition = 0.25;
        this.isRotatingLeft = false;
        this.isRotatingRight = false;
        this.rotationSpeed = 0.0225;
    ...
    initElements() {
        ...
        this.titleAccent = document.querySelector('#main-menu .title-accent');
    ...
    updateVolumePercents() {
        ...
        sliders.forEach(slider => {
            ...
            slider.style.background = `linear-gradient(90deg, #d85a48 0%, #d85a48 ${slider.value}%, rgba(255, 255, 255, 0.12) ${slider.value}%, rgba(255, 255, 255, 0.12) 100%)`;
        });
    }
    ...
    bindEvents() {
        ...
        if (this.versionLabel) {
            this.versionLabel.addEventListener('click', () => this.handleVersionSecretClick());
        }
    ...
    handleVersionSecretClick() {
        if (this.epsteinModeUnlocked) {
            return;
        }

        this.epsteinSecretClickCount += 1;

        if (this.epsteinSecretClickCount >= 17) {
            this.epsteinModeUnlocked = true;
            this.applyEpsteinModeState();
        }
    }

    applyEpsteinModeState() {
        if (!this.mainMenu || !this.titleAccent) {
            return;
        }

        this.mainMenu.classList.toggle('epstein-mode-unlocked', this.epsteinModeUnlocked);
        this.titleAccent.classList.toggle('epstein-mode-title', this.epsteinModeUnlocked);
    }
        ...
        if (type === 'night2') {
            ...
            tutorialContent.innerHTML = `
                ...
                <p>
                    ...
                    TRUMP CAN BE LURED WITH THE AUDIOS BUT YOUR MAIN PRIORITY WITH THE AUDIO LURES SHOULD BE VINNY.
                </p>
            ...
            // Night 1 教程：EP
            tutorialContent.innerHTML = `
                <h2>DEFEND YOURSELF AGAINST VINNY</h2>
                <p>
                    VINNY ALWAYS STARTS AT CAM 11. USE THE CAMERA'S AUDIO LURE TO KEEP VINNY FAR AWAY FROM YOU. 
                    MAKE SURE THE CAMERA YOU'RE PLAYING THE SOUND IN IS NEXT TO THE CAMERA WHERE VINNY IS. 
                    ...
                    VINNY DOES NOT ATTACK THROUGH THE VENTS SO DON'T BOTHER CLOSING THEM FOR THIS NIGHT.
                </p>
                <button id="tutorial-got-it">GOT IT</button>
            `;
            // 重新绑定按钮事件
    ...
    startViewRotation() {
        ...
        const rotationLoop = () => {
            ...
            this.rotationAnimationFrame = requestAnimationFrame(rotationLoop);
        };
    ...
    startFpsCounter() {
        if (!this.fpsDisplay || this.fpsAnimationFrame) {
            return;
        }
    ...
    playNight5VictoryAnimation() {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                rescueText.textContent = 'RESCUE ARRIVED';
                ...
                // 2秒后淡出 "RESCUE ARRIVED"，显示胜利画面
                setTimeout(() => {
                    rescueText.style.opacity = '0';
                    
                    setTimeout(() => {
                        // 移除 "RESCUE ARRIVED" 文字
