...
class InputHandler {
    ...
    handleKeyPress(e) {
        ...
        this.handleEpsteinSecretInput(e);
    ...
    handleEpsteinSecretInput(e) {
        if (!this.game.epsteinModeUnlocked || e.ctrlKey || e.metaKey || e.altKey) {
            return;
        }
        ...
        this.secretBuffer = (this.secretBuffer + e.key.toLowerCase()).slice(-5);

        if (this.secretBuffer === 'diddy') {
            this.secretBuffer = '';
            this.game.state.currentTime = 6;
            this.game.ui.update();
            this.game.winNight();
        }
    }
